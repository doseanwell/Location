...
def writeCsv(origin_sensors_data, outputs):
    sensor_file_name = "origin_sensors_data_" + str(
        origin_sensors_data[0]['gps3d'][6]) + ".csv"
    outputs_file_name = "Location_ouputs_" + str(
        origin_sensors_data[0]['gps3d'][6]) + ".csv"
    ...
    with open(sensor_file, "w", newline="") as sensor_csv_file:
        ...
        for i in range(len(origin_sensors_data)):
            if origin_sensors_data[i] != {}:
                ...
                new_gps = [
                    gps[1], gps[0], gps[2], 0.0, gps[4], gps[3], gps[5], 0.0,
                    0.0, 0.0, gps[6]
                ]
                data.extend(new_gps)
                data.extend(origin_sensors_data[i]["way2d"])
                all.append(data)
        print("Done! see files: \n" + str(sensor_file) + " size: " +
              str(len(all)) + "\n and \n" + str(outputs_file))
        write.writerows(all)
...
with open(
        "/home/sean/workspace/project/multi-nav/location/test/data/Sensor_record_20151030_105902_AndroSensor.csv"
) as f:
    data = f.readlines()
            ...
            if data[i].find("---") != -1 and data[i].find(">") == -1:
                ...
                if data[i].find("Output") != -1:
                    ...
                    if data[i].strip() != "" and data[i].find("=") != -1:
                        ...
                        line[key_values[0].strip()] = list(
                            map(lambda x: float(x),
                                key_values[1].strip().split(",")))
