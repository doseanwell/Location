...
void DataFormat::writeCSVs()
{
    ...
    std::string in_dir  = "/home/sean/workspace/project/multi-nav/location/test/datasensor_data";
    std::string out_dir = "/home/sean/workspace/project/multi-nav/location/test/datasensor_data_format";

    std::vector<std::string> files = getAllfiles(in_dir);

    for (auto &file : files) {
        std::ifstream inFile(in_dir + file);
        int file_cout = std::count(std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>(), '\n') + 1;
        inFile.close();
...
void DataFormat::writeCSV(std::string file_name, MatrixXd &acc, MatrixXd &g_data, MatrixXd &gyro, MatrixXd &mag,
                          MatrixXd &ornt_data, MatrixXd &gps_data)
{
    ...
    for (int i = 1; i < data_size; ++i) {
        ...
        outfile << gps_v(0) << "," << gps_v(1) << "," << gps_v(2) << "," << gps_v(3) << "," << gps_v(4) << ","
                << gps_v(5) << "," << gps_v(6) << endl;
    }
    std::cout << "write file: " << file_name << "data size is " << data_size << std::endl;
    outfile.close();
}

void DataFormat::readCSV(std::string file_name, MatrixXd &gyro, MatrixXd &acc, MatrixXd &mag, MatrixXd &gps_data,
                            ...
                            Vector3d &g_data, Vector3d &ornt_data, Vector3d &road_data)
{
    ...
    // printf("s_split[0] is:%s\n", s_split[0].c_str());
    // acc data.
    acc(0) = atof(s_split[0].c_str());
    acc(1) = atof(s_split[1].c_str());
    acc(2) = atof(s_split[2].c_str());
    // gravity data.
    g_data(0) = atof(s_split[3].c_str());
    g_data(1) = atof(s_split[4].c_str());
    g_data(2) = atof(s_split[5].c_str());
    // gyro data.
    gyro(0) = atof(s_split[6].c_str());
    gyro(1) = atof(s_split[7].c_str());
    gyro(2) = atof(s_split[8].c_str());
    //        gyro(i, 0) = atof(s_split[13].c_str());
    //        gyro(i, 1) = atof(s_split[14].c_str());
    //        gyro(i, 2) = atof(s_split[12].c_str());
    // mag data.
    mag(0) = atof(s_split[9].c_str());
    mag(1) = atof(s_split[10].c_str());
    mag(2) = atof(s_split[11].c_str());
    // gps data, gps(lng,lat,alt,accuracy,speed,bearing,t)
    double lng = atof(s_split[16].c_str());
    double lat = atof(s_split[15].c_str());
    ...
    gps_data(0) = lng;
    gps_data(1) = lat;
    gps_data(2) = atof(s_split[17].c_str());
    gps_data(3) = atof(s_split[20].c_str());
    ;
    //            gps_data(i, 3) = 10.0;
    gps_data(4) = atof(s_split[19].c_str());
    //            gps_data(i, 4) = atof(s_split[19].c_str()) * 1000.0 / 3600.0;
    gps_data(5) = atof(s_split[21].c_str());
    gps_data(6) = atof(s_split[25].c_str());

    //        ornt_data(2) = atof(s_split[12].c_str()) / M_PI * 180.0;
    //        ornt_data(0) = atof(s_split[13].c_str()) / M_PI * 180.0;
    //        ornt_data(1) = atof(s_split[14].c_str()) / M_PI * 180.0;

    ornt_data(2) = atof(s_split[12].c_str());
    ornt_data(0) = atof(s_split[13].c_str());
    ornt_data(1) = atof(s_split[14].c_str());

    //        heading(i) = atof(s_split[12].c_str());

    road_data(0) = atof(s_split[26].c_str());
    road_data(1) = atof(s_split[27].c_str());
    road_data(2) = atof(s_split[28].c_str());
