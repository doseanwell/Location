                                      ...
                                      Vector3d &g_data, Vector3d &ornt_data, Vector3d &road_data)
{
    ...
    if (!is_gps_valid) {
        ...
        if ((is_shaking || !is_compass_vaild) || (!is_near_cross && !is_routing)) {
            ...
            if (status.parameters.ins_count > status.parameters.Hz * status.parameters.least_gap_time_for_using_road) {
        ...
        if (is_ins_move_not_too_far) {
            ...
            // std::cout << "Current predict result: lng " << gps_new(0) << ", lat " << gps_new(1) << std::endl;
            // 更新经纬度
        ...
        // 更新其他INS变量
        // std::cout << "gps_count is:" << status.parameters.gps_count << std::endl;
    ...
    //                          + std::to_string(!is_near_cross) + " " +
    //                          std::to_string(status.parameters.dist_to_next_cross)
                                 ...
                                 Eigen::Vector3d &g_data, Eigen::Vector3d &ornt_data)
{
    ...
    if (pre_ornt_data(0) != -999.0 && pre_ornt_data(1) != -999.0 && pre_ornt_data(2) != -999.0) {
        ...
        input_data << acc_v_norm(0), acc_v_norm(1), acc_v_norm(2), acc_n_norm(0), acc_n_norm(1), acc_n_norm(2),
            g_v_norm(0), g_v_norm(1), g_v_norm(2), g_n_norm(0), g_n_norm(1), g_n_norm(2), gyro_v_norm(0),
            gyro_v_norm(1), gyro_v_norm(2), mag_v_norm(0), mag_v_norm(1), mag_v_norm(2), mag_v_diff(0), mag_v_diff(1),
            mag_v_diff(2), a_diff(0), a_diff(1), a_diff(2), ornt_diff(0), ornt_diff(1), ornt_diff(2);
        ...
        if (this->stopDetection) {
            ...
            if (cnt < total_detect_size) {
                ...
                if (stop_status_v.sum() / total_detect_size > 0.5)
                    this->status.parameters.stop_status = 0;
            }
        }
    }
    else {
...
void Location::AutoAdjustTFactor(Eigen::VectorXd &gps_data, double ins_distance)
{
    ...
    if (cnt < 3) {
        ...
        //        double gps_dist1 = gps_queue(0,4) * ((gps_queue(1, 6) - gps_queue(0, 6)) / 1000.0) + gps_queue(1, 4) *
        //        ((gps_queue(2, 6) - gps_queue(1, 6)) / 1000.0);
        ...
        if (gps_dist != 0.0 && ins_dist != 0.0 && deltaT != 0.0) {
            ...
            //                                  + std::to_string(deltaT) + " " + std::to_string(sqrt(gps_dist /
            //                                  ins_dist)) + " "
 ...
 * @param road_data, 道路方向数据,包含距离下个路口距离,和当前点的瞬时方向,以及当前道路类型编码, v(distance, heading,
 * code)
 */
 ...
 * code)
 */
void Location::UpdateZaxisWithRoad(Eigen::Vector3d &ornt_data, Eigen::Vector3d &road_data)
{
    static MatrixXd ornt_queue(this->status.parameters.queue_gps_ornt, 3);
    static MatrixXd road_queue(this->status.parameters.queue_gps_ornt, 3);
    static int      road_ornt_cnt = 0;
...
bool Location::IsRoadCompassSameRange(Vector3d &ornt_data, Vector3d &road_data)
{
    ...
    if (road_ornt_cnt < this->status.parameters.queue_road_ornt_len) {
        ...
        // 更新道路数据
        if ((road_data(0) != 0.0 || road_data(1) != 0.0)
            && (road_queue(road_ornt_cnt - 1, 1) != road_data(1) || road_queue(road_ornt_cnt - 1, 0) != road_data(0))) {
...
bool Location::IsOffCourse(Eigen::Vector3d &ornt_data, Eigen::Vector3d &road_data)
{
    ...
    // 判断是否以及行走到路口一定范围内
    bool is_near_cross = this->status.parameters.dist_from_pre_cross < this->status.parameters.min_dist_from_pre_cross
