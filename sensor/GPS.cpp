...
bool GPS::IsGPSBelongToTrack(routing::Status *status, Eigen::VectorXd &gps_data)
{
    ...
    if (cnt < (*status).parameters.gps_track_len) {
        ...
        //        std::cout << "cur_error <= (*status).parameters.weak_gps " << (cur_error <=
        //        (*status).parameters.weak_gps)
        //                  << " "
        ...
        if (cur_error <= (*status).parameters.weak_gps) {
            ...
            if (gps_data(6) == pre_gps_t) {
                ...
                if ((*status).parameters.road_type == 1.0) {
                    // 误差不可接受, 同时处于隧道状态,
                    // 则不考虑时间间隔,同时不拿下个点作为初始了,因为下个点可能是仍在隧道,但有次数限制
...
{
    // std::cout << "gps_data is:" << *gps_data << std::endl;
    // 精度是否足够
    ...
    bool   is_gps_move_accepted =
        ceil((*status).parameters.ins_count / 10.0 + 1.0) * gps_move_dist
        < ceil((*status).parameters.ins_count / 10.0 + 1.0) * (*status).parameters.move_distance_threshod;
    ...
    // std::cout << "is_gps_accuracy is:" << is_gps_accuracy << "\nis_gps_not_null is:" << is_gps_not_null
    //           << "\nis_gps_move_accepted is:" << is_gps_move_accepted
    //           << "\nis_gps_not_duplicated is:" << is_gps_not_duplicated
    //           << "\nis_gps_belong_to_track is:" << is_gps_belong_to_track
    //           << "\nis_gps_initializing is:" << is_gps_initializing
    //           << "\n(is_gps_static || is_gps_still_static) is:" << (is_gps_static || is_gps_still_static) <<
    //           std::endl;
    return (is_gps_accuracy && is_gps_not_null && is_gps_move_accepted && is_gps_not_duplicated
            && is_gps_belong_to_track)
           || is_gps_initializing
           || (is_gps_static || is_gps_still_static) && is_gps_belong_to_track;  // && is_gps_belong_to_track
}